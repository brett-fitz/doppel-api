import httpx
import pytest
import respx

from doppel.client import AsyncDoppel
from doppel.models.brands import BrandsListResponse

from .conftest import BASE_URL


class TestAsyncBrandsResource:
    @pytest.mark.asyncio
    @respx.mock(base_url=BASE_URL)
    async def test_list(self, respx_mock: respx.MockRouter, async_client: AsyncDoppel):
        respx_mock.get("/brands").mock(
            return_value=httpx.Response(
                200,
                json={
                    "data": [{"id": "b-1", "name": "Acme", "brand_type": "corporate"}],
                    "count": 1,
                },
            )
        )
        result = await async_client.brands.list()
        assert isinstance(result, BrandsListResponse)
        assert result.count == 1
        assert result.data[0].name == "Acme"

    @pytest.mark.asyncio
    @respx.mock(base_url=BASE_URL)
    async def test_list_with_filters(self, respx_mock: respx.MockRouter, async_client: AsyncDoppel):
        route = respx_mock.get("/brands").mock(
            return_value=httpx.Response(200, json={"data": [], "count": 0})
        )
        await async_client.brands.list(brand_type="corporate", limit=10)
        assert route.called
        assert "brand_type" in str(route.calls.last.request.url)
